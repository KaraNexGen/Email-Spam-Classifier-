"""
ESIS Clean Version - No Unicode Characters
=========================================
A clean version of ESIS that works without Unicode issues
"""
...
class SimpleESIS:
    ...
    def _extract_all_features(self, row: pd.Series) -> Dict[str, Any]:
        ...
        # Threat indicators (convert threat_level to numeric)
        for key, value in text_analysis['threat_indicators'].items():
            if key == 'threat_level':
                ...
                features[f'threat_{key}'] = float(threat_levels.get(value, 0))
            else:
    ...
    def train_models(self, X: np.ndarray, y: np.ndarray):
        ...
        # For MultinomialNB, we need non-negative values
        # So we'll use different scaling for different models
        X_train_scaled = self.scaler.fit_transform(X_train)
        X_test_scaled = self.scaler.transform(X_test)
        
        # For MultinomialNB, use MinMaxScaler to get non-negative values
        ...
        for name, model in models_config.items():
            ...
            # Use different scaling for MultinomialNB
            if name == 'naive_bayes':
        ...
        # Create ensemble (use scaled data for most models)
        ...
        # For ensemble, we need to handle the different scalings
        # We'll use the scaled version and create a wrapper for MultinomialNB
        ...
        # Create wrapper for MultinomialNB
        nb_wrapper = MultinomialNBWrapper(self.models['naive_bayes'], minmax_scaler)
    ...
    def save_model(self, filepath: str = 'esis_clean_model.pkl'):
    ...
    def load_model(self, filepath: str = 'esis_clean_model.pkl'):
...
def main():
    ...
    print("Email Security Intelligence System (ESIS) - Clean Version")
    print("=" * 70)
    ...
    print(f"Model saved as 'esis_clean_model.pkl'")
